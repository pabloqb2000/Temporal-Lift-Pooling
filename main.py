...
class Processor():
    def __init__(self, arg):
        ...
        shutil.copy2('./dataset/dataloader_video.py', self.arg.work_dir)
        ...
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.optimizer_args['start_epoch'], self.arg.num_epoch):
                ...
                seq_train(self.data_loader['train'], self.model, self.optimizer,
                          self.device, epoch, self.recoder, loss_weights= self.arg.loss_weights)
                if eval_model:
    ...
    def load_data(self):
        ...
        dataset_list = zip(["train", "train_eval", "dev", "test"], [True, False, False, False]) if 'phoenix' in self.arg.dataset else zip(["train", "dev"], [True, False])
        for idx, (mode, train_flag) in enumerate(dataset_list):
            ...
            self.dataset[mode] = self.feeder(gloss_dict=self.gloss_dict, kernel_size= self.kernel_sizes, **arg)
